...
describe("AllModelsTab", () => {
  ...
  describe("URL state", () => {
    ...
    it("falls back to Current Team Models for an unknown ?view=", () => {
      renderTab({}, "?view=bogus");
    ...
    it("writes the access group filter to ?filter_access_group=, resets the page, and drops it with its chip", async () => {
      const user = userEvent.setup();
      setModelsInfo([makeRow()], 100);
      const { onUrlUpdate } = renderTab({}, "?page=2");
